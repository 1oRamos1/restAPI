...
function MiniProgressBar({ currentLevel, progressScore }) {
  ...
  return (
    <div className="w-full mb-8">
      ...
      <div className="flex justify-between mb-2">
        {LEVELS.map((l, i) => (
          <div key={l.key} className="flex flex-col items-center" style={{ width: '33%' }}>
            <span className={`font-bold ${i === idx ? `text-sm ${i === 0 ? 'text-blue-500 dark:text-cyan-300' : i === 1 ? 'text-purple-500 dark:text-cyan-300' : 'text-yellow-500 dark:text-cyan-300'}` : 'text-xs text-gray-400 dark:text-cyan-600'}`}>
              {l.emoji} {l.label}
            </span>
          </div>
        ))}
      ...
      {/* Bar */}
      <div className="relative h-3 bg-gray-200 dark:bg-cyan-300/20 rounded-full overflow-hidden">
